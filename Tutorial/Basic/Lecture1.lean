...
example (h : P → Q) (hP : P) : Q := by
  ...
  -- 揃える必要があることに注意しよう。
  apply h hP  -- MEMO: ならばの適用方法の説明があるといいのかも
...
example (h : P → Q) (h' : Q → R) : P → R := fun hP ↦ h' (h hP)
...
example : ¬¬¬P → ¬P := by
  have neg_intro : P → ¬¬P := by
    intro hP hP'
    apply hP' hP
  intro hP'''
  intro hP
  apply hP''' (neg_intro hP) -- MEMO: P → ¬¬Pの定義の仕方がわからない 
